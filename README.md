...
│   ├── *.xlsx              # 你的週報檔案
│   └── en/*.json           # 英文對照檔（與 xlsx 同名），以「序號」對應每則新聞
...
```json
{
  ...
  "news": {
    "1": { "entity": "…", "title": "…", "summary": "…", "metric": "…", "impact": "…",
           "conf": "Confirmed (…)", "vocab": [["yield", "良率"], ["ramp", "（產能）爬坡"]] }
  }
}
```

由 `foundry-weekly-tools/build_weekly.py` 產生：內容 JSON 加上與 `news` 逐列對應的 `news_en`，
產生器會同時寫出 Excel 與 `data/en/` 對照檔。Excel 本身格式不變。
